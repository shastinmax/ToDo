...
import {v1} from 'uuid';
import {AddTodolistActionType, GetTodosType, RemoveTodolistActionType} from './todolists-reducer';
...
export const tasksReducer = (state: TasksStateType = initialState, action: ActionsType): TasksStateType => {
    switch (action.type) {
        ...
        case "GET-TODOS": {
            ...
            action.payload.todos.forEach(t => stateCopy[t.id] = [])
            return stateCopy
        }
        ...
        case "SET-TASKS": {
            const copyState = {...state};
            copyState[action.payload.todoId] = action.payload.tasks
            return copyState
        }
        default:
            return state;
...
export const getTaskThunkTC = (todolistId: string) => (dispatch: Dispatch) => {
...
export const updateTaskStatusTC = (taskId: string, status: TaskStatuses, todolistId: string) => (dispatch: Dispatch, getState: () => AppRootStateType) => {
    ...
    const allTask = allAppState.tasks
